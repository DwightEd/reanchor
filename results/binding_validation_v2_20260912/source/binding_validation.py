    ...
    def _capture(self, model, tokenizer, case, item, layout, subject, world):
        ...
        for pair in ([15, 16], [30, 31]):
            ...
            readouts[str(pair[-1])] = source_context_readout(
                attention, item["ids"], roles, len(item["ids"]) - 1, candidates
            )
...
def summarize(rows):
    ...
    for layout in sorted({r["layout"] for r in rows}):
        ...
        for layer in ("16", "31"):
            ...
            values = dict(
                common_valid=len(common), actual_errors=len(errors), methods={}, template_pairs=[]
            )
            for method in methods:
                ...
                values["methods"][method] = dict(
                    ...
                    error_correct=sum(method_correct(r, layer, method) for r in errors),
                )
