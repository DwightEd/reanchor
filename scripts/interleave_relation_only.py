...
def run_locked(args):
    tag = "relation" if args.experiment == "relation_only" else args.experiment
    journal = ROOT / f"runs/{tag}_interleave_20260913.json"
    mechanism_log = ROOT / f"runs/{args.experiment}_20260913.log"
    resume_name = (
        "ragtruth_population_resume_20260913.log"
        if args.experiment == "relation_only"
        else f"ragtruth_population_resume_{tag}_20260913.log"
    )
    resume_log = ROOT / "runs" / resume_name
    ...
    try:
        ...
        with mechanism_log.open("x") as log:
            child = subprocess.Popen(
                ["bash", f"scripts/run_{args.experiment}.sh", "--output", str(args.output)],
                cwd=ROOT,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--experiment", choices=("relation_only", "relation_routing"), default="relation_only"
    )
