...
2026-09-19：两个项目已完成新一轮证据核对，见
[当前分工与收敛方向](docs/CONVERGENCE_20260919.md)。
主线是候选内容与对象/阶段适用条件的功能一致性；下方旧结构—熵检测说明属于历史基线。
原生功能流与双头联合干预使用graph的`python -u main.py flow`，读取这里的原始采样。
尚无已验证的通用无监督绑定检测器，不将模型读到source等同于正确应用约束。
